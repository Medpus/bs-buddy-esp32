...
The JC3248W535 uses the **ESP32-S3 native USB** (VID:PID `303A:1001`), so no
CH340/CP2102 driver is needed.
...
in the `dialout` group it won't be writable. The fastest persistent fix is a udev rule:
...
The ROM bootloader can't be overwritten, so you can always recover:
